...
namespace kevintrinh1227.Methods {
    public class WelcomeMethod {
        ...
        public async Task WelcomeSendMethod(DiscordClient sender, DiscordGuild guild, DiscordMember member) {
            ...
            var serverlogsCh = guild.Channels.FirstOrDefault(x => x.Value.Name.ToLower().Contains("network-updates")).Value;
            ...
            var welcomeDMEmbed = new DiscordEmbedBuilder() {
                ...
                $"\n \n {warnEmo} **Account Linking** {warnEmo} \n ➜ Most importantly be sure to link your discord account with your Minecraft account by running `/discord link` in lobby/hub and sending your code here. Perks for server boosting can be found here {nitroCh.Mention}.",
                Color = new DiscordColor(0xFF0000),
                Timestamp = DateTime.UtcNow,
                Footer = new DiscordEmbedBuilder.EmbedFooter {
                    IconUrl = houndLogo.Url,
